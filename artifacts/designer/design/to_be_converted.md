...
Old-format design content that has not yet been fully converted into the formal entity style. Much of this content has already been superseded by formal definitions in other design files, but is preserved here for reference during conversion of any remaining unique details.
...
## Objects
Are selectable and come with an information panel when selected by player

### Selectable - true

### InformationPanel - player => struct
###
...
* Head Disjointed Attack
  * The Head Disjointed attack is an attack in which the Firing Phase is short and spawns a projectile animation which is executed independently of the Attacking unit's further actions. The projectile tracks the target and cannot miss. The Unit enters the Cooldown and Reloading Phases while the effect of the attack is delayed until the projectile reaches the target.
  * Head Disjointed attacks can not miss.
  * Units with Head Disjointed attacks cannot be used to force-target the ground and can only target specific units.
* Tail Disjointed Attack
  * The Tail Disjointed attack is an attack in which both the Animation and the Effect of the attack are applied to a target location which is the location of the attack's target at the end of the Aiming phase. The unit remains locked during the Firing phase. The effect of the attack will be applied to whatever units are in the original target's location at the end of the Firing phase.
  * Tail Disjointed attacks can be dodged by moving out of the target location.
  * Units with Tail Disjointed attacks may intentionally target specific locations.
* Doubly Disjointed Attack
  * Combination of Head and Tail disjointed.
  * After a short firing phase which spawns a projectile and frees the Attacking unit, the projectile travels to the location the target was at at the end of the Aiming phase. The effect of the attack is applied to whatever units are at that location when the projectile arrives.
  * Doubly Disjointed attacks can be dodged by moving out of the target location.
  * Units with Doubly Disjointed attacks may intentionally target specific locations.

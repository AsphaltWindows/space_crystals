...
mod tests {
    ...
    fn attack_target_has_indicator() {
        assert!(command_has_indicator(&UnitCommand::AttackTarget(Entity::from_raw(1))));
    }
    ...
    fn enter_has_indicator() {
        assert!(command_has_indicator(&UnitCommand::Enter(Entity::from_raw(1))));
    }

    #[test]
    fn pick_up_supplies_has_no_indicator() {
        assert!(!command_has_indicator(&UnitCommand::PickUpSupplies(Entity::from_raw(1))));
    }

    #[test]
    fn attach_to_tower_has_no_indicator() {
        assert!(!command_has_indicator(&UnitCommand::AttachToTower(Entity::from_raw(1))));
    }
    ...
    fn enter_color_is_green() {
        let color = command_indicator_color(&UnitCommand::Enter(Entity::from_raw(1)));
        assert_eq!(color, Color::srgb(0.0, 1.0, 0.0));
    }

    #[test]
    fn attack_target_color_is_red() {
        let color = command_indicator_color(&UnitCommand::AttackTarget(Entity::from_raw(1)));
    ...
    fn command_indicator_component_creation() {
        let indicator = CommandIndicator {
            owner_unit: Entity::from_raw(1),
            indicator_type: CommandIndicatorType::Location,
            target_entity: None,
            patrol_index: 0,
        };
        assert_eq!(indicator.owner_unit, Entity::from_raw(1));
    ...
    fn command_indicator_object_type() {
        ...
        assert_eq!(indicator.target_entity, Some(Entity::from_raw(2)));
    }

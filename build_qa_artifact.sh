...
ARTIFACT_DIR="$PROJECT_ROOT/qa_artifacts/$TIMESTAMP"
mkdir -p "$ARTIFACT_DIR"

# Copy binary
BINARY_SRC="$PROJECT_ROOT/target/debug/space_crystals"
...
ln -sfn "$TIMESTAMP" "$PROJECT_ROOT/qa_artifacts/latest"

# Success output
echo ""
echo "Build artifact saved to: qa_artifacts/$TIMESTAMP/"
echo "Symlink updated:         qa_artifacts/latest -> $TIMESTAMP"
echo ""
echo "Manifest:"
cat "$ARTIFACT_DIR/manifest.txt"
echo ""
echo "To run the game:"
echo "  cd $PROJECT_ROOT && ./qa_artifacts/latest/space_crystals"
echo ""
echo "IMPORTANT: Run from the project root so Bevy can find the assets/ directory."
